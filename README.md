...
| タブ | できること |
|------|------------|
| 車両 | 複数台を登録・編集・削除。名前 / メーカー / 型式 / ナンバー / 初期走行距離 / 動力源 (ガソリン / EV) / タンクまたはバッテリー容量。デフォルト車両を 1 台選択 |
| 給油 | 給油・充電の追加 (ステップ式ウィザード) ・編集。`満タン法` で区間燃費 / 電費 (km/L もしくは km/kWh) と平均値を自動計算 |
...
満タン法を採用しています。EV 車両 (動力源 = 電気) の場合は同じロジックで `km/kWh` を算出します。

- 満タン (または満充電) を基準点とし、次の満タン/満充電までの走行距離とエネルギー量から効率を算出
- 途中で部分給油 / 部分充電があっても、累積したエネルギー量に正しく含めて計算
- 各車両ごとの「平均燃費 / 平均電費」は最初の給油 (充電) を基準点として除外したうえで算出
